...
You can also run easylayout as a function and attribute the results to a variable,
containing the layout itself:

```{r layout_func, eval = FALSE}
layout <- easylayout(g)
```

Once you achieve your desired layout, retrieve the
results back to your R environment and use a plotting package to display the resulting network.
In the example below, we set the x and y values to node attributes and plot the
network using ggraph.
